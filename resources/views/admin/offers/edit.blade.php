            ...
            <form method="POST" action="{{ route("admin.offers.update", [$offer->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name"
                           id="name" value="{{ old('name', $offer->name) }}" required>
                    @if($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text"
                           name="description" id="description" value="{{ old('description', $offer->description) }}"
                           required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('category') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($categories as $id => $category)
                            <option
                                value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $offer->category->id ?? '') == $id ? 'selected' : '' }}>{{ $category }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('add_date') ? 'is-invalid' : '' }}" type="text"
                           name="add_date" id="add_date" value="{{ old('add_date', $offer->add_date) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('date_end') ? 'is-invalid' : '' }}" type="text"
                           name="date_end" id="date_end" value="{{ old('date_end', $offer->date_end) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('phone_number') ? 'is-invalid' : '' }}" type="text"
                           name="phone_number" id="phone_number" value="{{ old('phone_number', $offer->phone_number) }}"
                           required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('location') ? 'is-invalid' : '' }}" type="text"
                           name="location" id="location" value="{{ old('location', $offer->location) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number"
                           name="price" id="price" value="{{ old('price', $offer->price) }}" step="0.01" required>
                    @if($errors->has('price'))
                        <div class="invalid-feedback">
                            {{ $errors->first('price') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('city') ? 'is-invalid' : '' }}" name="city_id"
                        ...
                        @foreach($cities as $id => $city)
                            <option
                                value="{{ $id }}" {{ (old('city_id') ? old('city_id') : $offer->city->id ?? '') == $id ? 'selected' : '' }}>{{ $city }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('city'))
                        <div class="invalid-feedback">
                ...
                <div class="form-group" id="div_trader_id" style="display: none;">
                    ...
                    <select class="form-control select2 {{ $errors->has('trader') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($traders as $id => $trader)
                            <option
                                value="{{ $id }}" {{ (old('trader_id') ? old('trader_id') : $offer->trader->id ?? '') == $id ? 'selected' : '' }}>{{  $id . ' - '. $trader }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('show_in_main_offers_page') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="checkbox" name="show_in_main_offers_page"
                            ...
                            {{ old('show_in_main_offers_page', $offer->show_in_main_offers_page) == 1 ? 'checked' : '' }}>
                        <label class="required form-check-label"
...
@section('scripts')
    ...
    [
        'token'=>$token,
        'component_id'=>isset($offer)?$offer->trader_id:'0',
