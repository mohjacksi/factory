...
@section('content')
    ...
    <div class="form-group">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.products.variants.update", [$product,$variant->id]) }}" enctype="multipart/form-data">
                @method('PUT')
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('size') ? 'is-invalid' : '' }}" name="size_id" id="size_id">
                        @foreach($sizes as $id => $size)
                            <option value="{{ $id }}" {{ (old('size_id') ? old('size_id') : $variant->size->id ?? '') == $id ? 'selected' : '' }}>{{ $size }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('size'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('color') ? 'is-invalid' : '' }}" name="color_id" id="color_id">
                        @foreach($colors as $id => $color)
                            <option value="{{ $id }}" {{ (old('color_id') ? old('color_id') : $variant->color->id ?? '') == $id ? 'selected' : '' }}>{{ $color }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('color'))
                        <div class="invalid-feedback">
                ...
                <div class="form-group">
                    <label class="required" for="image">{{ trans('cruds.variant.fields.image') }}</label>
                ...
                <div class="form-group">
                    <label class="required" for="price">{{ trans('cruds.variant.fields.price') }}</label>
                    <input class="form-control  {{ $errors->has('price') ? 'is-invalid' : '' }}" type="text" name="price" id="price" value="{{ old('price', $variant->price) }}" required>
                    @if($errors->has('price'))
                        <div class="invalid-feedback">
                            {{ $errors->first('price') }}
                        </div>
                ...
                <div class="form-group">
                    <label class="required" for="count">{{ trans('cruds.variant.fields.count') }}</label>
                    <input type="number" class="form-control {{ $errors->has('count') ? 'is-invalid' : '' }}" name="count" id="count" required value="{{ old('count', $variant->count) }}">
                    @if($errors->has('count'))
                        <div class="invalid-feedback">
                            {{ $errors->first('count') }}
                        </div>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('is_available') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="checkbox" name="is_available" id="is_available" value="1"
                            {{ old('is_available', $variant->is_available) == 1 ? 'checked' : '' }}>
