...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.sub_product_type.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sub_product_types.update", [$sub_product_type->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.sub_product_type.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $sub_product_type->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('main_product_type_id') ? 'is-invalid' : '' }}" name="main_product_type_id" id="main_product_type_id"  required>
                    @foreach($main_product_types as $id => $value)
                        <option value="{{ $id }}" {{ in_array($id, old('value', [$sub_product_type->MainProductType->id])) ? 'selected' : '' }}>{{ $value }}</option>
                    @endforeach
                </select>
