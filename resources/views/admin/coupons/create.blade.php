...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form id="myForm" method="POST" action="{{ route("admin.coupons.store") }} " enctype="multipart/form-data">
                @csrf
                <div class="form-group" id="div_trader_id" >
                    ...
                    <select class="form-control select2 {{ $errors->has('trader') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($traders as $id => $trader)
                            <option
                                value="{{ $trader->id }}" {{ old('trader_id') == $trader->id ? 'selected' : '' }}>{{  $trader->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('max_usage_per_user') ? 'is-invalid' : '' }}"
                           type="number"
                           name="max_usage_per_user" id="max_usage_per_user" value="{{ old('max_usage_per_user', '') }}"
                           required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('min_total') ? 'is-invalid' : '' }}" type="number"
                           name="min_total" id="min_total" value="{{ old('min_total', '') }}" >
                    ...
                    @foreach(App\Models\Coupon::TYPE_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('type') ? 'is-invalid' : '' }}">
                            <input class="form-check-input type" type="radio" id="type_{{ $key }}" name="type"
                                   value="{{ $key }}" {{ old('type', '') === (string) $key ? 'checked' : '' }} required>
                ...
                <div class="form-group fixed_discount" style="display: none;">
                    ...
                    <input class="form-control {{ $errors->has('fixed_discount') ? 'is-invalid' : '' }}" type="text"
                           name="fixed_discount" id="fixed_discount" value="{{ old('fixed_discount', 0) }}" >
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('expire_date') ? 'is-invalid' : '' }}" type="text"
                           name="expire_date" id="expire_date" value="{{ old('expire_date') }}" required>
