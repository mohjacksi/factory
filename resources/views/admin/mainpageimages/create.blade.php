...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.mainpageimages.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                <label class="required" for="city_id">{{ trans('cruds.item_advertisement.fields.city_name') }}</label>
...
@if(isset($advertisement) && $advertisement->images)
      var files = {!! json_encode($advertisement->images) !!}
          for (var i in files) {
          var file = files[i]
