            ...
            <form method="POST" action="{{ route("admin.departments.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('item_number') ? 'is-invalid' : '' }}" type="text"
                           name="item_number"
                           id="item_number" value="{{ old('item_number', '') }}" required>
                ...
                <div class="form-group"  id="div_trader_id" style="display: none;">
                    ...
                    <select class="form-control select2 {{ $errors->has('trader') ? 'is-invalid' : '' }}"
                            name="trader_id" id="trader_id" style="display: none;">
                        @forelse($traders as $id => $trader)
                            <option
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('show_in_main_page') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="checkbox" name="show_in_main_page" id="show_in_main_page"
                               value="1"
                            {{ old('show_in_main_page', 0) == 1 || old('show_in_main_page') === null ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('show_in_main_departments_page') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="checkbox" name="show_in_main_departments_page" id="show_in_main_departments_page"
                               value="1"
                            {{ old('show_in_main_departments_page', 0) == 1 || old('show_in_main_departments_page') === null ? 'checked' : '' }}>
                        <label class="required form-check-label"
                    ...
                    @endif
                    <span class="help-block">{{ trans('cruds.department.fields.show_in_main_departments_page_helper') }}</span>
                </div>
...
@section('scripts')
    ...
    @include('admin.departments.components.form_scripts',
    ['token'=>$token,'sub_category_id'=>isset($deparment)?$deparment->sub_category_id:'0'])

    @include('admin.departments.components.trader_city_form_scripts',
    [
        'token'=>$token,
        'component_id'=>isset($department)?$department->city_id:'0',
